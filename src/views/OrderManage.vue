...
const dishColumns: TableProps['columns'] = [
  ...
  },
  {
    ...
    width: 80,
    align: 'center',
    cell: (h: any, { row }: any) => `x${row.num || row.qty || 1}`
  },
  {
    colKey: 'price',
    title: '單價',
    width: 120,
    align: 'right',
    cell: (h: any, { row }: any) => formatMoney(row.price)
  }
]
...
async function fetchOrders() {
  ...
  const statusList = statusFilter.value.length > 0 ? statusFilter.value.map(String) : null
  const beginTime = dateRange.value?.[0] ? `${dateRange.value[0]} 00:00:00` : null
  const endTime = dateRange.value?.[1] ? `${dateRange.value[1]} 23:59:59` : null
  const orderId = searchKeyword.value || null

  const res = await ServerApi.OrderInfo.getAllOrderInfoByShopIdStaffUse(
    currentPage.value,
    pageSize.value,
    statusList,
    beginTime,
...
async function showOrderDetail(orderId: string) {
  ...
  const res = await ServerApi.OrderInfo.getDishOrderById(
    orderId,
    (error) => {
...
function getTagProps(tag: any): TagProps {
  ...
  const theme = getStatusTheme(tag) as 'default' | 'primary' | 'warning' | 'danger' | 'success'
  return {
    theme,
    variant: 'light'
  }
