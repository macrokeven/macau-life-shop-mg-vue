...
const notify = {
  warning(message: string) {
    MessagePlugin.warning(message, 1500)
  },
  error(message: string) {
    MessagePlugin.error(message, 2000)
  },
  success(message: string) {
    MessagePlugin.success(message, 2000)
  },
  info(message: string) {
    MessagePlugin.info(message, 2000)
  }
}
...
const LetoyService = {
  /**
   * 发起需要授权的请求（带 token）
   * @param url - API 路径（不需要 /api 前缀）
   * @param method - 请求方法 GET/POST/PUT/DELETE
   * @param params - URL 参数（GET 请求使用）
   * @param data - 请求体数据（POST/PUT 请求使用）
   * @param fallCallBack - 错误回调
   * @param onFinalCallBack - 最终回调（无论成功失败都执行）
   * @returns 返回响应数据
   */
  async authorizationRequest<T = any>(
    url: string,
    method: 'GET' | 'POST' | 'PUT' | 'DELETE' = 'GET',
    params: Record<string, any> | null = null,
    data: Record<string, any> | null = null,
    fallCallBack: ((error: AxiosError) => void) | null = null,
    onFinalCallBack: (() => void) | null = null
    ...
    try {
      ...
      if (fallCallBack && typeof fallCallBack === 'function') {
        fallCallBack(error as AxiosError)
      }
      return null
    } finally {
      // 执行最终回调
   ...
   * @param data - 请求体数据
   * @param fallCallBack - 错误回调
   * @param onFinalCallBack - 最终回调
   * @returns 返回响应数据
   */
  async noAuthorizationRequest<T = any>(
    url: string,
    method: 'GET' | 'POST' | 'PUT' | 'DELETE' = 'GET',
    params: Record<string, any> | null = null,
    data: Record<string, any> | null = null,
